...
const reducer = (state: VoiceState, { type, payload }: VoiceReducer): VoiceState => {
    switch(type) {
        case VoiceReducerActionType.SET_AUDIO:
            return { ...state, audio: payload as Exclude<VoiceReducerActionPayload, MediaRecorder | boolean> }
        case VoiceReducerActionType.SET_RECORDER:
            return { ...state, recorder: payload as Exclude<VoiceReducerActionPayload, Blob | boolean> }
        case VoiceReducerActionType.SET_IS_RECORDING:
            return { ...state, isRecording: payload as Exclude<VoiceReducerActionPayload, Blob | null | MediaRecorder> }
        default:
            return state
    }
}
